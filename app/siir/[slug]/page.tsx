...
export async function generateMetadata({ params }: Props) {
  ...
  const item = await getItemBySlug(slug, "poem");
  if (!item) return {};
  return pageMetadata({
    title: item.title,
...
export default async function SiirPage({ params }: Props) {
  const { slug } = await params;
  const item = await getItemBySlug(slug, "poem");
  if (!item || item.kind !== "poem") notFound();

  const path = `/siir/${slug}`;
    ...
    <>
      ...
      <article className="site-section">
        ...
        <section className="mt-8 rounded-lg bg-stone-100 p-6" aria-label="Özet">
          ...
          <p className="text-stone-800 leading-relaxed">{item.excerpt}</p>
          <p className="mt-4 text-sm text-stone-600">
            Bu şiir, {AUTHOR_NAME}&apos;nin {BOOK_TITLE} adlı şiir kitabının
            bir parçasıdır. Tam metin kitabı satın aldıktan sonra dijital
            okuyucuda okunabilir.
          </p>
        </section>
